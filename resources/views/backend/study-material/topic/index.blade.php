                        ...
                        <div class="box">
                            ...
                            <div class="box-header with-border">
                                <div class="d-flex align-items-center">
                                    ...
                                    @can('createStudyMaterialTopic', Auth::user())
                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        @if($topics->isNotEmpty())
                                            @foreach($topics as $topic)
                                                <tr>
                                                    <td>{{ $topic->title }}</td>
                                                    <td>{{ $topic->parent?->title }}</td>
                                                    <td>{{ $topic->topicGroup?->name }}</td>
                                                    <td>{{ $topic->educationLevel?->name }}</td>
                                                    <td>{{ $topic->subject?->name }}</td>
                                                    <td class="text-center">
                                                        @if($topic->status === \App\Enums\Status::ACTIVE->value)
                                                            <span class="badge badge-success">
                                                                {{ ucfirst(strtolower(\App\Enums\Status::from($topic->status)->name)) }}
                                                            </span>
                                                        @else
                                                    ...
                                                    <td class="text-center">
                                                        @can('updateStudyMaterialTopic', Auth::user())
                                                            <a href="{{ route('admin.topics.edit', [$topic]) }}">
                                                                <i class="fa fa-edit" aria-hidden="true"></i>
                                                            </a>
                                                        @endcan
                                                        @can('deleteStudyMaterialTopic', Auth::user())
                                                            <button type="button"
                                                                    data-route="{{ route('admin.topics.destroy', [$topic]) }}"
